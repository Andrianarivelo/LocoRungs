...
for f in range(len(foldersRecordings)):
    ...
    for r in range(len(foldersRecordings[f][2])):
        (existenceDLC2, DLC2_file) = eSD.getDLC2TrackingFiles(mouse, foldersRecordings[f][0], foldersRecordings[f][2][r][:-4] + '-' + foldersRecordings[f][2][r][-3:])
        if existenceDLC2:
            tracking_paths.append(DLC2_file)




paws_data = np.column_stack((pd.read_hdf(tracking_paths[0]).values[:, [0, 1, 3, 4, 6, 7, 9, 10]], np.arange(
    pd.read_hdf(tracking_paths[0]).values.shape[0])))  # import HDF5 file into numpy array
